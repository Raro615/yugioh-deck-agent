...
### alias 컬럼에는 뜻이 두 가지 있다

`cards.cdb` 의 `alias` 는 "같은 카드의 다른 일러스트" 이기도 하고
**"룰상 저 카드명으로 취급"** 이기도 하다. 후자는 전혀 다른 카드다.

| 카드 | alias | 관계 |
|---|---|---|
| 伝説の都 アトランティス | 海 | 이름이 「海」로 취급될 뿐, **다른 카드** |
| ハーピィ・レディ2 | ハーピィ・レディ | 이름 취급, **다른 카드** |
| Lovely Labrynth (다른 일러스트) | 원본 | **같은 카드** |

구분하지 않고 cid 를 물려주면 엉뚱한 카드의 공식 재정이 붙는다. 그래서
`expand_aliases()` 는 **카드명이 같을 때만** 물려준다 (일본어명이 양쪽에 있으면
그것으로, 없으면 영어명으로 대조). 이름 비교는 링크를 **거부**하는 데만 쓰고,
링크를 만들어내는 데는 쓰지 않는다.
