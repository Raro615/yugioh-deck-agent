...
- **Phase 2-AG** — 수를 보고 다음 일을 할지 정한다 (``execution.py`` ·
  ``effect/definition.py``). STRUCTURAL-91 · 92.

  **비교는 둘뿐이다.** 조작의 결과를 조건으로 쓰는 자리 97곳을 세어 보니
  **91곳이 0 과 견준다** (``>0`` 57 · ``==0`` 26 · ``~=0`` 8). 장수는
  음수가 될 수 없으므로 ``>0`` · ``~=0`` · ``>=1`` 은 같은 질문이고
  ``==0`` · ``<=0`` 도 같은 질문이다. ``AT_LEAST`` 와 ``AT_MOST`` 로
  96곳이 적히고, ``EQUAL`` 이나 ``GREATER_THAN`` 은 쓰는 카드를 못 찾아
  만들지 않았다 — 정확히 N 은 둘을 함께 걸면 된다.

  **세 답이 세 가지 다른 일로 이어진다.** 참이면 하고, 거짓이면
  **건너뛰고**(실패가 아니다, 효과는 계속된다), 모르면 **전체를
  거절한다.** 마지막이 요점이다 — 건너뛰는 것도 결정이고, 모르는 채로
  건너뛰면 그 일을 했어야 하는지를 엔진이 지어낸 것이 된다.

  건너뛴 일은 **자리를 지킨다**. 뒤의 일이 번호로 앞을 가리키므로
  번호가 밀리면 안 되고, 그 자리에는 ``NOT_APPLIED`` 가 들어간다.
  ``FAILED`` 는 여전히 없다. 건너뛴 일에는 **장수가 없다** — 0 이
  아니다.

  2-AE 와 부딪히지 않는다. ``OperationRequirement`` 는 앞이 **규칙대로
  되었는가**를 보고 아니면 거절하며, ``OperationGuard`` 는 앞의 **수가
  조건을 만족하는가**를 보고 아니면 건너뛴다. 2-AE 가 막은 것은 벌거벗은
  수를 참/거짓으로 읽는 것이고, 여기서는 수를 수로 읽고 견주는 방법을
  명시적으로 적는다.

  **조건은 계획 시점의 판을 본다.** 같은 효과 안에서 앞의 일이 바꾼
  판은 보이지 않으므로, 앞의 일이 한 것을 보려면 판이 아니라 **결과**를
  가리켜야 한다 (STRUCTURAL-94 로 적어 두었다).

  부수적으로 **0 을 누가 거부하는가**가 제자리를 찾았다. 계산된 0 은
  고르라는 자리에서는 틀렸지만 **조건의 입력으로는 멀쩡한 값**이므로,
  그 규칙을 값이 아니라 **쓰는 자리**로 옮겼다.
  ``docs/phase2ag-conditional-flow.md`` 참고.

- **Phase 2-AH** — 계획이 내다보는 판 (``state/game_state.py`` ·
  ``effect/executor.py``). STRUCTURAL-94.

  **진단이 먼저 뒤집혔다.** "계획 시점에는 앞선 결과가 없다" 는 이
  엔진에서 사실이 아니다 — 결과는 Phase 2-AD 부터 계획 루프 안에서
  확정된다. 없던 것은 결과가 아니라 **판**이었다: 계획이 읽는 판이 효과
  시작 시점에 멈춰 있었다.

  그래서 **실제 버그가 있었다.** 상대 패에서 무작위로 한 장씩 두 번 고르는
  효과가 **같은 카드를 두 번 골랐고**, "두 장을 보냈다" 고 기록하면서 한
  장만 움직였다. 최소 재현을 먼저 만들어 확인한 뒤 고쳤다.

  ``GameState.project()`` 는 ``clone()`` 과 **한 가지만** 다르다 —
  난수원을 복제하지 않고 **함께 쓴다.** 난수는 계획 단계에서 꺼내고
  꺼낸 횟수가 재현의 좌표이므로, 투영이 제 난수원을 가지면 그 좌표가
  진짜 판에 남지 않아 같은 seed 가 다른 판이 된다. **판의 모양은
  복제되고 흐름의 위치는 공유된다** — 해시에서 난수원을 뺀 것과 같은
  가름이다.

  계획은 조작을 확정할 때마다 **투영에** 반영하고, 진짜 판은 계획이 전부
  끝난 뒤에야 바뀐다. 전부 아니면 무가 그대로이고, 덤으로 적용 도중의
  예외가 계획 단계에서 잡혀 **판이 반쯤 바뀌는 길이 하나 줄었다.**

  Promise/Future 계층은 **만들지 않았다.** 대신 수가 **왜 없는지**를
  넷으로 갈랐다 — 아직 닿지 않았다 · 하지 않았다(2-AG 건너뜀) · 그 칸이
  없다 · 있다. 셋 다 "수가 없다" 로 끝나지만 고쳐야 할 것이 다르고, 셋
  다 ``UNKNOWN``("규칙을 판정할 수 없다")과 다르다. 그중 "그 칸이 없다"
  는 **적는 순간 아는 거짓말**이므로 정의를 만들 때 막는다.

  **고리는 만들어질 수 없다.** 참조가 언제나 앞을 가리켜야 하므로 조작
  순서가 이미 위상 정렬이고, 그래서 고리를 푸는 장치를 두지 않았다.
  ``docs/phase2ah-plan-projection.md`` 참고.

- **Phase 2-AI** — 고르지 않는 선택과 첫 실제 카드 (``effect/target.py`` ·
  ``effect/executor.py`` · ``activation.py`` · ``effect/library.py``).
  STRUCTURAL-10.

  **새 계층을 만든 자리가 아니다.** 2-AD ~ 2-AH 가 만든 사슬을 실제 카드
  하나에 처음으로 태운 자리다. 감사 결과가 그 동기다: 그때까지 등재된
  15장 중 **guard · requirement · declaration · FROM_RESULT 를 쓰는 카드가
  하나도 없었다.** 그 계층들은 synthetic 정의로만 돌고 있었다.

  막고 있던 것은 대상 요구의 모양이었다. "패를 **전부** 덱으로" 를
  ``CHOOSING`` 으로 적으려면 ``minimum``/``maximum`` 을 **지금 후보
  수**에 맞춰야 하고, 그러면 명세가 판을 설명하는 것이 아니라 판을 베낀
  것이 된다. 그래서 ``TargetRequirement.ALL`` 과 ``AllMatchingSpec`` 을
  더했다 — **장수 칸도 고르는 사람 칸도 두지 않았다.** 전부는 수가 아니라
  범위이고, 없는 칸이 곧 그 주장이다.

  한 장을 위한 구조 변경이 아니라는 근거는 측정이다. 그룹을 고르지 않고
  통째로 넘기는 스크립트가 **1,156장**(몬스터 692 · 마법 247 · 함정 217),
  그중 필터조차 없는 것이 **176장**이다.

  ``ALL`` 은 ``RANDOM`` 과 **같은 자리에서 갈라진다.** 둘 다 아무도 고르지
  않으므로 밖에서 고른 결과를 받지 않고, 발동이 아니라 해결 중에 정해지며,
  후보는 **자리마다 그 주인의 눈**으로 센다. 다른 점은 난수를 쓰지 않는
  것뿐이다.

  등재한 실제 카드는 리로드(22589918) 하나다. ``@primary`` 가 패 전부,
  0번이 덱으로 되돌리기, 1번이 셔플, 2번이 ``ResultRef(0, ATTEMPTED_COUNT)``
  만큼 드로우 — 뽑는 수를 정의에 적어 두지 않았다는 것이 요점이다.
  ``docs/phase2ai-bulk-selection.md`` 참고.
