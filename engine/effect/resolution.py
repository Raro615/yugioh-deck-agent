...
class ResolutionStatus(str, Enum):
    ...
    **``RESOLVED`` 하나만 판을 바꾼다.** 나머지는 전부 "아무 일도 일어나지
    ...
    """
    고른 대상이 **적법한지 판정할 수 없어** 수행하지 않았다. 판은 그대로다.

    가려진 정보 때문이다 — 관측에 없는 카드이거나, 후보 조건이 카드 정의를
    요구하는데 뒷면이거나, 후보를 찾을 존이 통째로 가려져 있거나.

    ``INVALID_TARGET`` 과 합치지 않는다. 그쪽은 **확실히 틀린** 대상이고
    이쪽은 **모르는** 대상이다. 모르는 것을 틀린 것으로 적으면, 정보가
    생겼을 때 왜 답이 바뀌는지 설명할 수 없다.
    """
