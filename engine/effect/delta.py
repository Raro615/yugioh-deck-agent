...
class SummonKind(str, Enum):
    ...
    반전 소환 · 제물 소환은 그 절차가 생길 때 함께 들어온다. 계층이 없는
    이름을 미리 못박으면 나중에 실제 모양과 어긋난다
    ...
    SPECIAL = "special"
    """
    특수 소환 (Phase 2-T). **어떤 특수 소환법인가는 말하지 않는다.**

    융합 · 싱크로 · 엑시즈 · 링크 · 의식 · 펜듈럼은 각자의 절차가 생길 때
    자기 이름을 갖는다. 지금 그 이름들을 미리 만들어 두면, 절차 없이 이름만
    있는 칸이 "이 소환은 융합이다" 라고 주장하게 된다 — 일반 소환 하나로
    시작했던 것과 같은 이유다.

    "어떤 카드가 특수 소환되었는가" 와 "어떤 방법으로" 는 다른 질문이고,
    지금 답할 수 있는 것은 앞의 것뿐이다 (STRUCTURAL-62).
    """
