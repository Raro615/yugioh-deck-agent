...
```
입력
 ├─ 1. 전체 입력이 실제 카드명과 정확히 일치?  → 카드명 검색으로 확정
 ├─ 2. 아니면 게임 용어 / 조건 파싱
 └─ 3. 해석 못 한 조각이 남고 이름 일부와 맞으면 → 이름 검색
```

| 입력 | 처리 |
|---|---|
| `마법족의 마을` | 1단계. 실제 카드명 → 68462976 1장 |
| `마법사족의 마을` | 2단계. 그런 카드명 없음 → 마법사족 794장, `마을` 은 미인식으로 보고 |
| `마법사족` | 2단계. 종족 필터 |
| `푸른 눈의 백룡` | 1단계. 1종 (판본 8개) |
